...
def eigenvectors(W, W_br=None, in_labels=None, out_label=None, opts=None):
    ...
    if n==1:
        ...
        plt.tick_params(
            ...
            labelbottom=False) # labels along the bottom edge are False
        plt.axis([1, m, -1, 1])
...
def sufficient_summary(y, f, out_label=None, opts=None):
    ...
    plt.rc('font', **opts['myfont'])
    ax1.plot(y1, f, 'bo', markersize=12)
    ax1.set_xlabel('Active variable')
    ax1.set_ylabel(out_label)
    ax1.set_title('Sufficient Summary')
    ax1.grid(True)
    if opts['savefigs']:
        figname = 'figs/ssp1_' + out_label + opts['figtype']
        fig1.savefig(figname, dpi=300, bbox_inches='tight', pad_inches=0.0)
    ...
    if n==2:
        ...
        ax2.axis([ymin, ymax, ymin, ymax])
        img1 = ax2.scatter(y1, y2, c=f, s=150.0, vmin=np.min(f), vmax=np.max(f))
        fig2.colorbar(img1, ax=ax2)
        ax2.set_xlabel('Active variable 1')
        ax2.set_ylabel('Active variable 2')        
        ax2.set_aspect('equal')
        ax2.grid(True)
        ax2.set_title(out_label)
        if opts['savefigs']:
            figname = 'figs/ssp2_' + out_label + opts['figtype']
            fig2.savefig(figname, dpi=300, bbox_inches='tight', pad_inches=0.0)
